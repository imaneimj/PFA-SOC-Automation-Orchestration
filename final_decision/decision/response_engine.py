def _build_context(normalized,risk,ml_result=None):
    risk=risk or {}
    ml_result=ml_result or {}
    ml_analysis=normalized.get("ml_analysis",{}) or {}
    return {
        ...
        "confidence":(normalized.get("classification",{}) or {}).get("confidence"),
        "final_score":risk.get("final_score",0),
        "malicious_iocs":risk.get("malicious_iocs",0),
        "ips":normalized.get("ioc",{}).get("ips",[]),
        "urls":normalized.get("ioc",{}).get("urls",[]),
        "domains":normalized.get("ioc",{}).get("domains",[]),
        "hashes":normalized.get("ioc",{}).get("hashes",[]),
        "ml_available":ml_result.get("available",False),
        "ml_prediction":ml_result.get("prediction"),
        "ml_confidence":ml_result.get("confidence"),
        "ml_proba_true_positive":ml_result.get("proba_true_positive"),
...
def _build_targeted_actions(incident,platform,hostname,normalized,context):
    actions=[]
    ...
    elif incident in ("Credential Access","Execution"):
        actions.append({
            "tool":"velociraptor",
            "action":"run_artifact",
            "response":"KILL_PROCESS",
...
def build_response(decision,normalized,risk=None,ml_result=None):
    incident=normalized.get("incident_type","")
    platform=normalized.get("platform","Linux")
    hostname=normalized.get("hostname","")
    context=_build_context(normalized,risk,ml_result)
